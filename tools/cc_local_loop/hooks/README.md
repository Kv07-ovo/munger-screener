...
- 快照写入前一律经 `redact` 脱敏：正常走 `common.redact`；即便 `common.py` 无法导入，
  也使用本地 **fallback redact**（标准库正则，与 `common.redact` 保持一致：覆盖
  `sk-`/`ghp_`/`hf_`/`xox`/`AKIA`、`Bearer`、`api_key=`/`token=`/`password=`/`secret=`，
  以及**带下划线的环境变量名** `ANTHROPIC_API_KEY=` / `AWS_SECRET_ACCESS_KEY=` /
  `GITHUB_TOKEN=` 等），**绝不是 no-op**，确保任何降级路径下密钥都不会落入快照。
